...
library(ggplot2)
library(xts)
data("prcp_cropped.tif") #This loads the data included in the package, but you would attach your own

# Extract 1 spatial point from the raster data
infile = prcp_cropped.tif
...
Now that we have the key dates and a filtered timeseries, it is time to calculate the statistics related to the MSD. We will see an output of 1 value per year of data. Because the timeseries we've created from the raster is for 5 years of data (from 1981 to 1985) we will see 5 values of each parameter.
...
duration <- apply(filterTS2, MARGIN = 2, FUN = msdStats, keyDatesTS, fcn="duration")
intensity <- apply(filterTS2, MARGIN = 2, FUN = msdStats, keyDatesTS, fcn="intensity")
firstMaxValue <- apply(filterTS2, MARGIN = 2, FUN = msdStats, keyDatesTS, fcn="firstMaxValue")
secondMaxValue <- apply(filterTS2, MARGIN = 2, FUN = msdStats, keyDatesTS, fcn="secondMaxValue")
min <- apply(filterTS2, MARGIN = 2, FUN = msdStats, keyDatesTS, fcn="min")
```
